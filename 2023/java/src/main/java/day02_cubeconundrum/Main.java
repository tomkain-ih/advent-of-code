...
//        System.out.println("Part 2 Input Sum: " + powerSum);
    ...
    private static List<String> readInput() {
        InputStream inputStream = day01_trebuchet.Main.class.getClassLoader().getResourceAsStream(INPUT);
        assert inputStream != null;
        return new BufferedReader(new InputStreamReader(inputStream)).lines().toList();
    }

    private static final Map<String, Integer> PART_ONE_LIMITS = Map.of(
            "red", 12,
            "green", 13,
            "blue", 14
    );

    private static final Map<Long, Long> PART_TWO_EXPECTED_POWERS =
            Map.of(1L, 48L, 2L, 12L, 3L, 1560L, 4L, 630L, 5L, 36L);
